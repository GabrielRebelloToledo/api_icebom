...
class CapturaComposicaoService {
    ...
    async buscarProdutosPorOps(id) {
        const sql = ` SELECT ID_INVENTARIO_ITENS,ID_INVENTARIO, COD_PRO,  DESC_PRO, QUANT, LOCAL_ARMAZENAGEM, ESTOQUE, DIFERENCA, TIPO_ITEM, SIMB_UND FROM INVENTARIO_ITENS_ESTOQUE WHERE ID_INVENTARIO = ? ORDER BY ID_INVENTARIO_ITENS ASC`;
