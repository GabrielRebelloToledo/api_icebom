...
class StatusController {
  ...
  async create(request, response) {
    ...
    const { status, color, colorText } = request.body;

    console.log(status, color, colorText)

    const createStatus = container.resolve(CreateService);
    const cstatus = await createStatus.execute({ status, color, colorText });
  ...
  async update(request, response) {
    ...
    const ustatus = await updateStatus.execute({ id, status, color, colorText });
